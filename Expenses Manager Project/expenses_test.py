...
class Expenses_Test(unittest.TestCase):
    ...
    def test_get_expense(self):
        ...
        # test non-existing expense types
        self.assertEqual(None, expenses_manager.get_expense(self.expenses, "internet"))

        # test getting expenses based on existing expense types
        self.assertEqual(5.00, expenses_manager.get_expense(self.expenses, "food").amount)
    ...
    def test_add_expense(self):
        ...
        expenses_manager.add_expense(self.expenses, "coffee", 84.5)
        self.assertAlmostEqual(96.9, self.expenses.get("coffee").amount)

        expenses_manager.add_expense(self.expenses, "clothes", 200)
        self.assertAlmostEqual(245, self.expenses.get("clothes").amount)
    ...
    def test_deduct_expense(self):
        ...
        with self.assertRaises(RuntimeError):
            expenses_manager.deduct_expense(self.expenses, "entertainment", 200)

        #   Test deducting from non-existing expense
        expenses_manager.deduct_expense(self.expenses, "hair", 20)
        self.assertRaises(TypeError, expenses_manager.deduct_expense(self.expenses, "hair", 20))
    ...
    def test_update_expense(self):
        ...
        #  Hint(s):
        #   Test updating an expense
        expenses_manager.update_expense(self.expenses, "coffee", 12.4)
        self.assertAlmostEqual(12.4, expenses_manager.get_expense(self.expenses, "coffee").amount)

        #   Test updating a non-existing expense
        expenses_manager.update_expense(self.expenses, "transport", 19.99)
        self.assertRaises(TypeError, expenses_manager.get_expense(self.expenses, "transport"))
    ...
    def test_export_expenses(self):
        ...
        # testing with non-existent expense types and duplicate expense types.
        file1 = 'export2.txt'
        expense_types1 = ['coffee', 'clothes', 'transport', 'coffee']
        expenses_manager.export_expenses(self.expenses, expense_types1, file1)

        # read the exported file for file1
        f = open(file1)
        lines = f.readlines()
        f.close()
        self.assertEqual('coffee: 12.40', lines[0].strip())
        self.assertEqual('coffee: 12.40', lines[2].strip())
        self.assertEqual('clothes: 45.00', lines[1].strip())
